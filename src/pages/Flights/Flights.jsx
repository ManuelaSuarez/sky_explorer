...
const Flights = () => {
  ...
  const getMatchingFlights = () => {
    ...
    if (searchFrom) {
      flights = flights.filter((flight) => flight.origin.toLowerCase().includes(searchFrom.toLowerCase()))
    }

    if (searchTo) {
      flights = flights.filter((flight) => flight.destination.toLowerCase().includes(searchTo.toLowerCase()))
    }
  ...
  useEffect(() => {
    getFlights()
    fetchReviews() // Also fetch reviews on page load
  }, [])
